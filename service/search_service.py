...
async def do_searches(job_id_mongo, action: ActionSearchHome):
    ...
    await send_results(research_to_send, n_homes, job, action)
    logging.info("end")


async def send_results(research_to_send: [], n_homes: int, job: Job, action: ActionSearchHome):
    repository = Repository()
